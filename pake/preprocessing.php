<?php
	...
	class Preprocessing
	{
		...
		public function pecah_kalimat($text)
		{
			// $this->num_docs = count($text);
			// for ($index=0; $index < $this->num_docs; $index++) { 
			// 	$doc_index = array();
			// 	$doc_index = explode(".", $text[$index]);

			// 	$num_index = count($doc_index);
			// 	for ($position=0; $position < $num_index; $position++) { 
			// 		$kal = strtolower($doc_index[$position]);
			// 		$this->corpus_terms[$kal][] = array($index, $position);
			// 	}
			// }
		...
		public function preproses($text)
		{
		...
		public function tf($text) {
			$data = $this->preproses($text);
			foreach ($data as $key) {
				$wordCount = array_count_values($key);
			}
			return $wordCount;
		}
	...
	while ($data = mysql_fetch_array($select)) {
		...
		foreach ($pecah as $key => $hasil) {
			...
			foreach ($ir->preproses($hasil) as $keyPre => $valPre) {
				// echo $valPre."<br>";
				$hasil_stem = $stem->stemming($valPre);
				echo $hasil_stem."<br>";
			}
			echo "<br><br>";
		}
