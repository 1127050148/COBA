<?php
	class Keywords
	{
		public function getKeywords($string, $nbrwords = 5)
		{
			...
			$data = str_word_count($string, 1);
			array_walk($data, array($this, 'filter'));
			$data = array_diff($data, $stopword2);
			...
			$jumlahWC = count($wordCount);
			foreach ($wordCount as $wc => $vwc) {
				echo "<tr><td>$wc</td><td>$vwc</td></tr>";
			}
			echo "</table>";
			echo "<b>Jumlah Kata : " .$jumlah3. "</b><br>";
			echo "---------------------------------------------";
			$wordCount = array_slice($wordCount, 0 , $array_stopword);
			return array_keys($wordCount);
		}

		private function filter($vwc, $wc)
		{
			$vwc = strtolower($vwc);
		}
		...
		{
			$this->stopword2 = array();
		}
	}

	$test = new Keywords();
	$key = $test->getKeywords($string, $jumlah3)
?>